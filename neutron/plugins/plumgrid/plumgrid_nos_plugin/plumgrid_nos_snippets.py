...
"""
PLUMgrid Platform implements an extended data model
in order to provide Distributed Virtual Topologies.
These are snippets needed by the PLUMgrid Platform.
"""
...
class DataDirectorPLUMgrid():
    ...
    def create_cdb_topology(self, tenant_id):
        return self.CDB_BASE_URL + tenant_id + "/domain/neutron-based"
    ...
    def create_link_url(self, tenant_id, prefix_link_id, sufix_link_id=""):
        link_path = "/link/" + prefix_link_id[:6] + sufix_link_id[:6]
        return self.BASE_Director_URL + tenant_id + link_path

    def create_link_body_data(self, ne_start, ne_end, ifc_name_ne_start="1",
                              ifc_name_ne_end="1"):
        ne_start = "/ne/" + ne_start + "/ifc/" + ifc_name_ne_start[:6]
        ne_end = "/ne/" + ne_end + "/ifc/" + ifc_name_ne_end[:6]
        body_data = {"link_type": "static",
                     "attachment1": ne_start,
                     "attachment2": ne_end}
        return body_data

    def create_gen_link_body_data(self, first_ne_name, second_ne_name,
                                  first_ifc_name, second_ifc_name):
        ne_start = "/ne/" + first_ne_name + "/ifc/" + first_ifc_name
        ne_end = "/ne/" + second_ne_name + "/ifc/" + second_ifc_name
        body_data = {"link_type": "static",
                     "attachment1": ne_start,
                     "attachment2": ne_end}
        return body_data
    ...
    def create_bridge_body_data(self, tenant_id, bridge_name):
        body_data = {"ne_type": "bridge",
                     "mobility": "true",
                     "ne_dname": bridge_name,
                     "ifc": {"1": {"ifc_type": "static"}},
                     "action": {"action1":
                                    {"action_text":
                                         "create_and_link_ifc(DYN_)"}},
    ...
    def create_router_body_data(self, tenant_id, router_name):
        body_data = {"ne_type": "router",
                     "mobility": "true",
                     "ne_dname": router_name,
                     "action":
                         {"action1":
